...
//   return (
//     <div className={`flex flex-col min-h-screen ${currentTheme.bgColor} ${currentTheme.textColor} font-sans`}>
//       {/* Navbar */}
//       <nav className={`${currentTheme.navBgColor} ${currentTheme.navTextColor} fixed w-full top-0 z-10 shadow-md`}>
//         <div className="max-w-7xl mx-auto px-4 py-4 flex justify-between items-center">
//           <h1 className="text-xl font-bold">{data.name}'s Portfolio</h1>
...
//               <button
//                 onClick={() => setShowThemeMenu(!showThemeMenu)}
//                 className={`${currentTheme.secondaryColor} px-3 py-1 rounded hover:opacity-80`}
//               >
//                 Theme
//               </button>
//               {showThemeMenu && (
...
//                         currentTheme.id === theme.id ? `${theme.accentColor} font-bold` : ''
//                       }`}
//                     >
//                       {theme.name}
//                     </button>
...
//       {/* Main Content */}
//       <main className="flex-grow px-4 pt-20 pb-10 max-w-5xl mx-auto">
//         {/* About */}
//         <section id="about" className="py-8">
//           <h2 className={`text-3xl font-bold ${currentTheme.textColor} mb-3`}>About Me</h2>
//           <p className={`${currentTheme.cardTextColor}`}>
//             {data.About}
//           </p>
//         </section>
...
//           <h2 className={`text-3xl font-bold ${currentTheme.textColor} mb-3`}>Projects</h2>
//           <div className="grid grid-cols-1 gap-6 sm:grid-cols-2">
//             {(data.Project || []).map((project, idx) => (
//               <div key={idx} className={`${currentTheme.cardBgColor} p-6 rounded-lg shadow-md`}>
//                 <h3 className={`text-xl font-semibold ${currentTheme.cardTextColor}`}>{project.name}</h3>
//                 <p className={`${currentTheme.cardTextColor} mt-2`}>{project.description}</p>
//                 <p className={`${currentTheme.accentColor} mt-2`}>{project.year}</p>
//               </div>
//             ))}
//           </div>
//         </section>
...
//       {/* Footer */}
//       <footer className={`${currentTheme.navBgColor} ${currentTheme.navTextColor} py-4 text-center`}>
...
import { Link } from "react-router-dom";
import { HomeIcon } from "lucide-react"; // Adjust the import path as needed
const themes = [
  {
    id: "teal",
    name: "Teal Tide",
...
const Template1 = () => {
  ...
  return (
    <div
      className={`flex flex-col min-h-screen ${currentTheme.bgColor} ${currentTheme.textColor} font-sans`}
    >
      {/* Navbar */}
      <nav
        className={`${currentTheme.navBgColor} ${currentTheme.navTextColor} fixed w-full top-0 z-10 shadow-md`}
      >
        <div className="max-w-7xl mx-auto px-4 py-4 flex justify-between items-center">
          <Link to='/dashboard'>
                        <HomeIcon/>
          ...
          <div className="flex gap-4 items-center">
            <a
              href={`/dashboard/template-portfolio2/${id}#about`}
              className="hover:opacity-75"
            >
              About
            </a>
            <a
              href={`/dashboard/template-portfolio2/${id}#projects`}
              className="hover:opacity-75"
            >
              Projects
            </a>
      ...
      <main className="flex-grow px-4 pt-20 pb-10 max-w-5xl mx-auto">
        {/* About */}
        <section id="about" className="py-8">
          <h2 className={`text-3xl font-bold ${currentTheme.textColor} mb-3`}>
            About Me
          </h2>
        ...
        <section id="projects" className="py-8">
          ...
          <div className="grid grid-cols-1 gap-6 sm:grid-cols-2">
            {(data.Project || []).map((project, idx) => (
              <div
                key={idx}
                className={`${currentTheme.cardBgColor} p-6 rounded-lg shadow-md`}
              >
                <h3
                  className={`text-xl font-semibold ${currentTheme.cardTextColor}`}
                >
                  {project.name}
                </h3>
                ...
                <p className={`${currentTheme.accentColor} mt-2`}>
                  {project.year}
                </p>
              </div>
            ))}
      ...
      {/* Footer */}
      <footer
        className={`${currentTheme.navBgColor} ${currentTheme.navTextColor} py-4 text-center`}
      >
        <p>
